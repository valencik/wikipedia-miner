...
public class LabelSensesStep extends Configured implements Tool {
	...
	public enum Output {tempLabel, tempPageLink, tempCategoryParent, tempArticleParent, sentenceSplits, fatalErrors} ;
	...
	private static class LabelSensesMapper extends MapReduceBase implements Mapper<LongWritable, Text, Text, ExLabel> {
			...
			try {				
				...
				if (page != null) {
					...
					case category :
						...
						gatherCategoryLinks(page, markup, reporter) ;
					...
					// now emit all of the anchors we have gathered
		...
		private void processSentence(String sentence, int sentenceIndex, DumpPage page, HashMap<String, ExLabel> labels, TreeMap<Integer, ArrayList<Integer>> outLinksAndLocations, Reporter reporter) throws Exception {
			...
			for(int[] linkRegion: linkRegions) {
				...
				if (link != null && link.getTargetNamespace()==SiteInfo.MAIN_KEY && link.getTargetLanguage() == null) {
				...
				/*
				 * This doesn't make sense no that category links are gathered separately
				 
				if (link != null && link.getTargetNamespace()==SiteInfo.CATEGORY_KEY && link.getTargetLanguage() == null) {
		...
		private void gatherCategoryLinks(DumpPage page, String markup, Reporter reporter) throws Exception {
